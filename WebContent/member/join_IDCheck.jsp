...
<body>
	<%
		//join.jsp(작은창인? 현재 join_IDCheck.jsp를 오픈해준 창)
		//아이디 중복확인을 위해 입력한 아이디가 한글이라면 한글이 깨지므로 인코딩 방식을 UTF-8로 설정
		request.setCharacterEncoding("UTF-8");
	
		//1.join.jsp에서 입력한 아이디를 request영역에서 꺼내오기(요청값 얻기)
		//2.밑의 중복확인 버튼을 눌렀을때 <form>태그로부터 전달받은 userid값 가져오기 
		String id =request.getParameter("userid");
		
		//3.MemberDAO클래스에 .. 아이디중복 체크 유무 값 가져오기 DB작업을 위한 idCheck() 메소드 추가 
		
		//4.입력받은 id값이 db에 존재하는지 중복체크를 위한 idCheck()메소드 호출 시.. 매개변수로 전달
		//작업 후 아이디 중복 체크 유무값 반환받기 
		...
		if(check == 1){
			...
			//사용가능한 ID이면 사용함버튼을 눌러서 부모창(join.jsp)에서 사용가능한 ID를 출력해주기 
	%>
